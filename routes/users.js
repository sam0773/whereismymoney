...
router.get('/:username/settings', (req, res) => {
    ...
    db.get('SELECT * FROM userSettings WHERE username = ?', [username], (err, row) => {
        ...
        if (row) {
            res.json({
                overviewAmountHidden: row.overviewAmountHidden === 1,
...
router.put('/:username/settings', (req, res) => {
    const { username } = req.params;
    const { overviewAmountHidden, shareManagementEnabled, currentAmountQueryEnabled } = req.body;
    ...
    db.get('SELECT * FROM userSettings WHERE username = ?', [username], (err, row) => {
        ...
        if (row) {
            // 更新现有记录
            db.run(
                'UPDATE userSettings SET overviewAmountHidden = ?, shareManagementEnabled = ?, currentAmountQueryEnabled = ? WHERE username = ?',
                [overviewAmountHidden ? 1 : 0, shareManagementEnabled ? 1 : 0, currentAmountQueryEnabled ? 1 : 0, username],
                (err) => {
                    if (err) {
                        res.status(500).json({ error: err.message });
                        return;
            ...
            // 插入新记录
            db.run(
                'INSERT INTO userSettings (username, overviewAmountHidden, shareManagementEnabled, currentAmountQueryEnabled) VALUES (?, ?, ?, ?)',
                [username, overviewAmountHidden ? 1 : 0, shareManagementEnabled ? 1 : 0, currentAmountQueryEnabled ? 1 : 0],
                (err) => {
                    if (err) {
                        res.status(500).json({ error: err.message });
                        return;
