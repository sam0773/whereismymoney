...
router.post('/', (req, res) => {
    ...
    db.run(
        'INSERT INTO wealth (username, platform, name, type, riskLevel, transactionType, redeemType, date, expiryDate, amount, redeemedAmount, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
        [username, platform, name, type, riskLevel, transactionType, redeemType, date, expiryDate, amount, redeemedAmount, status],
        function(err) {
            if (err) {
...
router.put('/:id', (req, res) => {
    ...
    db.run(
        'UPDATE wealth SET platform = ?, name = ?, type = ?, riskLevel = ?, transactionType = ?, redeemType = ?, date = ?, expiryDate = ?, amount = ?, redeemedAmount = ?, status = ? WHERE id = ?',
        [platform, name, type, riskLevel, transactionType, redeemType, date, expiryDate, amount, redeemedAmount, status, id],
        function(err) {
            if (err) {
                res.status(500).json({ error: err.message });
                return;
