...
function switchTab(tabName) {
    ...
    if (tabName === 'overview') {
        ...
        setTimeout(async () => {
            ...
            // 确保金额显隐功能正常工作（等待设置加载完成）
            await initAmountToggle();
            // 应用当前的金额显示状态
            updateAmountsDisplay();
        }, 100);
    }
...
// 初始化金额显隐功能
async function initAmountToggle() {
        ...
        try {
            ...
            if (!currentUser) {
                ...
                if (response.ok) {
                    ...
                    amountsVisible = !settings.overviewAmountHidden;
                } else {
                    amountsVisible = false;
                }
            }
...
window.showAssetTypeDistribution = showAssetTypeDistribution;
window.calculateAssetTypeDistribution = calculateAssetTypeDistribution;
window.showPlatformComposition = showPlatformComposition;
window.calculatePlatformComposition = calculatePlatformComposition;
...
function updateAssetCompositionChart(depositTotal, fundTotal, wealthTotal, stockTotal, otherTotal = 0) {
    ...
    // 添加点击事件 - 点击资产构成显示该类型的资产分布
    assetCompositionChart.off('click');
    assetCompositionChart.on('click', function(params) {
        showAssetTypeDistribution(params.name);
    });
}
...
function updateAssetDistributionChart() {
    ...
    // 添加点击事件 - 点击资产分布显示该平台的资产构成
    assetDistributionChart.off('click');
    assetDistributionChart.on('click', function(params) {
        showPlatformComposition(params.name);
    });
}
...
async function initOverview() {
    ...
    // 先初始化金额显示设置（必须在 updateSummary 之前调用，避免闪烁）
    await initAmountToggle();
...
// 显示指定资产类型的分布饼图（点击资产构成饼图时调用）
function showAssetTypeDistribution(assetType) {
    // 创建模态框
    let modal = document.getElementById('assetTypeDistributionModal');
    if (!modal) {
        modal = document.createElement('div');
        modal.id = 'assetTypeDistributionModal';
        modal.className = 'modal';
        modal.innerHTML = `
            <div class="modal-content" style="max-width: 600px; width: 90%; padding: 20px;">
                <h2 id="assetTypeDistributionTitle">资产分布</h2>
                <div id="assetTypeDistributionChart" style="width: 100%; height: 400px;"></div>
                <div class="form-actions" style="margin-top: 20px;">
                    <button id="closeAssetTypeDistributionModal" class="btn btn-secondary">关闭</button>
                </div>
            </div>
        `;
    ...
    // 计算该资产类型的分布数据
    const distributionData = calculateAssetTypeDistribution(assetType);
    
    // 更新标题
    document.getElementById('assetTypeDistributionTitle').textContent = `${assetType}分布`;
    
    // 显示模态框
    modal.style.display = 'flex';
    
    // 初始化图表
    const chartDom = document.getElementById('assetTypeDistributionChart');
    ...
    const option = {
        ...
        color: [
            '#3498db', '#2ecc71', '#f1c40f', '#e74c3c', '#00bcd4',
            '#9b59b6', '#34495e', '#1abc9c', '#e67e22', '#e84393'
        ],
        series: [
            {
                name: assetType,
    ...
    // 关闭按钮事件
    document.getElementById('closeAssetTypeDistributionModal').onclick = function() {
...
// 计算指定资产类型的分布数据
function calculateAssetTypeDistribution(assetType) {
    const distribution = {};
    const today = new Date();
    ...
    switch(assetType) {
        case '定期存款':
            ...
            deposits.forEach(deposit => {
                ...
                if (expiryDate >= today) {
                    const bank = deposit.bank;
                    distribution[bank] = (distribution[bank] || 0) + parseFloat(deposit.amount || 0);
                }
            });
            break;
        ...
        case '基金':
            ...
            const fundGroups = {};
            funds.forEach(fund => {
                const key = `${fund.platform}_${fund.fundCode || ''}`;
                if (!fundGroups[key]) {
                    fundGroups[key] = { platform: fund.platform, totalBuy: 0, totalRedeem: 0, status: 'active' };
                }
                if (fund.transactionType === 'buy') {
                    fundGroups[key].totalBuy += parseFloat(fund.amount || 0);
                } else if (fund.transactionType === 'redeem') {
                    fundGroups[key].totalRedeem += parseFloat(fund.amount || 0);
                    if (fund.redeemType === 'full') {
                        fundGroups[key].status = 'redeemed';
                    }
                }
            });
            for (const [key, group] of Object.entries(fundGroups)) {
                if (group.status === 'active') {
                    const currentAmount = group.totalBuy - group.totalRedeem;
                    if (currentAmount > 0) {
                        distribution[group.platform] = (distribution[group.platform] || 0) + currentAmount;
                    }
                }
            }
            break;
        ...
        case '理财':
            ...
            wealths.forEach(wealth => {
                let key;
                if (wealth.type === '每日开放') {
                    key = `${wealth.platform}_${wealth.name}_${wealth.type}`;
                } else if (['最短持有期', '定期开放式', '固定天数循环', '结构性周期'].includes(wealth.type)) {
                    key = `${wealth.platform}_${wealth.name}_${wealth.type}_${wealth.cycleType || '未知'}`;
                } else {
                    key = `${wealth.id}_${wealth.date}`;
                }
                if (!wealthGroups[key]) {
                    wealthGroups[key] = { platform: wealth.platform, totalBuy: 0, totalRedeem: 0, status: 'active' };
                }
                if (wealth.transactionType === 'buy') {
                    wealthGroups[key].totalBuy += parseFloat(wealth.amount || 0);
                } else if (wealth.transactionType === 'redeem') {
                    wealthGroups[key].totalRedeem += parseFloat(wealth.amount || 0);
                    if (wealth.redeemType === 'full') {
                        wealthGroups[key].status = 'redeemed';
                    }
                }
            });
            for (const [key, group] of Object.entries(wealthGroups)) {
                if (group.status === 'active') {
                    const currentAmount = group.totalBuy - group.totalRedeem;
                    if (currentAmount > 0) {
                        distribution[group.platform] = (distribution[group.platform] || 0) + currentAmount;
        ...
        case '证券':
            ...
            for (const [broker, balance] of Object.entries(brokerBalances)) {
                if (!balance.isFullTransfer && balance.amount > 0) {
                    distribution[broker] = (distribution[broker] || 0) + balance.amount;
                }
            }
            break;
            
        case '其它':
            // 按平台汇总未全部赎回且投资金额>0的其它投资
            const others = window.currentOtherInvestments || [];
            const otherGroups = {};
            others.forEach(other => {
                const key = `${other.type}_${other.name}_${other.platform || '无平台'}`;
                if (!otherGroups[key]) {
                    otherGroups[key] = { platform: other.platform || '无平台', totalBuy: 0, totalRedeem: 0, status: 'active' };
                }
                if (other.transactionType === 'buy') {
                    otherGroups[key].totalBuy += parseFloat(other.amount || 0);
                } else if (other.transactionType === 'redeem') {
                    otherGroups[key].totalRedeem += parseFloat(other.amount || 0);
                    if (other.redeemType === 'full') {
                        otherGroups[key].status = 'redeemed';
                    }
                }
            });
            for (const [key, group] of Object.entries(otherGroups)) {
                if (group.status === 'active') {
                    const currentAmount = group.totalBuy - group.totalRedeem;
                    if (currentAmount > 0) {
                        distribution[group.platform] = (distribution[group.platform] || 0) + currentAmount;
...
// 显示指定平台的资产构成饼图（点击资产分布饼图时调用）
function showPlatformComposition(platform) {
    // 创建模态框
    let modal = document.getElementById('platformCompositionModal');
    if (!modal) {
        modal = document.createElement('div');
        modal.id = 'platformCompositionModal';
        modal.className = 'modal';
        modal.innerHTML = `
            <div class="modal-content" style="max-width: 600px; width: 90%; padding: 20px;">
                <h2 id="platformCompositionTitle">资产构成</h2>
                <div id="platformCompositionChart" style="width: 100%; height: 400px;"></div>
                <div class="form-actions" style="margin-top: 20px;">
                    <button id="closePlatformCompositionModal" class="btn btn-secondary">关闭</button>
                </div>
            </div>
        `;
        document.body.appendChild(modal);
    ...
    // 计算该平台的资产构成数据
    const compositionData = calculatePlatformComposition(platform);
    
    // 更新标题
    document.getElementById('platformCompositionTitle').textContent = `${platform}资产构成`;
    
    // 显示模态框
    modal.style.display = 'flex';
    
    // 初始化图表
    const chartDom = document.getElementById('platformCompositionChart');
    const chart = echarts.init(chartDom);
        ...
        series: [
            {
                ...
                radius: ['40%', '70%'],
                center: ['50%', '45%'],
                data: compositionData.filter(item => item.value > 0),
                emphasis: {
                    itemStyle: {
                        shadowBlur: 5,
                        shadowOffsetX: 0,
    ...
    // 关闭按钮事件
    document.getElementById('closePlatformCompositionModal').onclick = function() {
        modal.style.display = 'none';
        chart.dispose();
    };
...
// 计算指定平台的资产构成数据
function calculatePlatformComposition(platform) {
    const composition = {
        '定期存款': 0,
        '基金': 0,
        '理财': 0,
    ...
    const deposits = window.currentDeposits || [];
    deposits.forEach(deposit => {
        if (deposit.bank === platform) {
            const expiryDate = new Date(deposit.expiryDate || deposit.expiry_date);
            if (expiryDate >= today) {
                composition['定期存款'] += parseFloat(deposit.amount || 0);
            }
        }
    });
    ...
    const fundGroups = {};
    funds.forEach(fund => {
        if (fund.platform === platform) {
            const key = `${fund.platform}_${fund.fundCode || ''}`;
            if (!fundGroups[key]) {
                fundGroups[key] = { totalBuy: 0, totalRedeem: 0, status: 'active' };
            }
            if (fund.transactionType === 'buy') {
                fundGroups[key].totalBuy += parseFloat(fund.amount || 0);
            } else if (fund.transactionType === 'redeem') {
    ...
    wealths.forEach(wealth => {
        if (wealth.platform === platform) {
            ...
            if (!wealthGroups[key]) {
                wealthGroups[key] = { totalBuy: 0, totalRedeem: 0, status: 'active' };
            }
            if (wealth.transactionType === 'buy') {
                wealthGroups[key].totalBuy += parseFloat(wealth.amount || 0);
            } else if (wealth.transactionType === 'redeem') {
    ...
    // 4. 证券
    const transfers = window.currentStocks || [];
    const brokerBalance = { amount: 0, isFullTransfer: false };
    transfers.forEach(transfer => {
        if (transfer.broker === platform) {
            if (transfer.transferType === 'bankToSecurities' || transfer.type === '银行转证券') {
                brokerBalance.amount += parseFloat(transfer.amount || 0);
            } else if (transfer.transferType === 'securitiesToBank' || transfer.type === '证券转银行') {
                brokerBalance.amount -= parseFloat(transfer.amount || 0);
            }
            if (transfer.isFullTransfer) {
                brokerBalance.isFullTransfer = true;
            }
        }
    });
    if (!brokerBalance.isFullTransfer && brokerBalance.amount > 0) {
        composition['证券'] += brokerBalance.amount;
    }
    ...
    const otherGroups = {};
    others.forEach(other => {
        if (other.platform === platform || (platform === '无平台' && !other.platform)) {
            const key = `${other.type}_${other.name}`;
            if (!otherGroups[key]) {
                otherGroups[key] = { totalBuy: 0, totalRedeem: 0, status: 'active' };
            }
            if (other.transactionType === 'buy') {
                otherGroups[key].totalBuy += parseFloat(other.amount || 0);
            } else if (other.transactionType === 'redeem') {
