            ...
            if (transaction.transactionType === 'buy') {
                ...
                if (!currentBatch || currentBatch.status === 'redeemed') {
                    ...
                    currentBatch = {
                        ...
                        fundCode: transaction.fundCode, // 添加基金代码
                        name: transaction.name,
                        type: transaction.type,
                        fundCategory: transaction.fundCategory, // 添加基金类型
                        fundRiskLevel: transaction.fundRiskLevel, // 添加风险等级
                        transactions: [],
                        totalBuyAmount: 0,
                        totalRedeemAmount: 0,
                        currentAmount: 0,
                ...
                currentBatch.transactions.push({
                    ...
                    fundCategory: transaction.fundCategory, // 保存基金类型
...
async function renderFundTableSection(tableBodyId, fundList, tableType, sortedFund = null) {
    ...
    if (fundList.length === 0) {
            ...
            if (tableType === 'history') {
                colspan = 9; // 包含基金代码、基金类型和风险等级列
            } else {
                colspan = 8; // 包含基金代码、基金类型和风险等级列，减少了购买日期、购买金额、赎回日期、赎回金额列，增加了交易详情列
            }
    ...
    try {
        // 创建赎回记录
            const redeemRecord = {
                ...
                fundCategory: fund.fundCategory, // 保存基金类型
...
function showYieldCalculationFundModal(fund) {
    ...
    // 返回Promise，等待用户操作
    return new Promise((resolve) => {
        const calculateBtn = document.getElementById('calculateYieldFundBtn');
    ...
    contentEl.innerHTML = `
        ...
        <div style="margin: 10px 0; padding: 10px; background-color: #f5f5f5; border-radius: 5px;">
            <div style="display: flex; flex-wrap: wrap; gap: 20px; align-items: center;">
                <div><strong>基金类型:</strong> ${fund.fundCategory || '未设置'}</div>
            ...
            <tbody>
                ${sortedTransactions.map((transaction, index) => `
                    <tr>
                        ...
                        <td style="padding: 8px; border: 1px solid #ddd; text-align: center;">${transaction.type === 'buy' ? '购买' : '赎回'}</td>
    ...
    saveBtn.onclick = async () => {
        ...
        try {
            ...
            const response = await fetch(`/api/fund/${transactionId}`, {
                ...
                body: JSON.stringify({
                    platform: transaction.platform,
                    fundCode: transaction.fundCode, // 添加基金代码字段
                    name: transaction.name,
                    type: transaction.type,
...
function showMaintainFundInfoModal(fund) {
        ...
        modal.innerHTML = `
            <div class="modal-content">
                ...
                <form id="maintainFundInfoForm">
                    ...
                    <div class="form-group">
                        <label for="maintainFundCategory">基金类型:</label>
    ...
    queryBtn.onclick = async () => {
        ...
        try {
            ...
            const { fundName, fundCategory, fundRiskLevel } = fundInfo;
            ...
            if (confirm('基金信息查询成功！是否立即更新该基金代码下所有记录？')) {
                ...
                // 更新所有相关交易记录 - 只更新产品名称和风险等级，不更新基金类型
                for (const transaction of fundTransactions) {
                    await fetch(`/api/fund/${transaction.id}`, {
                        ...
                        body: JSON.stringify({
                            ...
                            name: fundName || transaction.name, // 更新产品名称
                            type: transaction.type,
                            transactionType: transaction.transactionType,
                            redeemType: transaction.redeemType,
                            date: transaction.date,
                            ...
                            redeemedAmount: transaction.redeemedAmount,
                            status: transaction.status,
                            fundCategory: transaction.fundCategory, // 不更新基金类型，保留原有值
                            fundRiskLevel: fundRiskLevel || transaction.fundRiskLevel // 更新风险等级
                        })
                    });
                }
        ...
        try {
            ...
            for (const transaction of fundTransactions) {
                await fetch(`/api/fund/${transaction.id}`, {
                    ...
                    body: JSON.stringify({
                        platform: transaction.platform,
                        fundCode: transaction.fundCode, // 保留原有基金代码
                        name: fundName, // 更新产品名称
                        type: transaction.type,
                        transactionType: transaction.transactionType,
                        redeemType: transaction.redeemType,
                        date: transaction.date,
                        ...
                        redeemedAmount: transaction.redeemedAmount,
                        status: transaction.status,
                        fundCategory: fundCategory, // 更新基金类型
                        fundRiskLevel: fundRiskLevel // 更新风险等级
                    })
                });
            }
...
function showFundMaintenanceList() {
    ...
    mergedFund.forEach(fund => {
        // 只根据基金代码去重，确保同一个基金代码只出现一次
        // 不会存在没有基金代码的基金，所以不需要额外处理
    ...
    if (fundProducts.length === 0) {
        tableHtml += `
            <tr>
                <td colspan="5" style="text-align: center; color: #999; padding: 20px;">暂无基金数据</td>
            </tr>
        `;
    } else {
        fundProducts.forEach(fund => {
            tableHtml += `
                <tr>
                    <td style="border: 1px solid #ddd; padding: 8px;">${fund.fundCode || ''}</td>
...
async function queryFundInfo() {
    ...
    try {
        ...
        // 基金类型不再根据代码查询变化，保持为空值
        // 不设置fundCategory，保持表单默认值
