    ...
    const sortedWealth = [...wealthList].sort((a, b) => {
        ...
        switch (column) {
            ...
            case 'type':
                ...
                break;
            case 'riskLevel':
                // 风险等级排序：R1 < R2 < R3 < R4 < R5
                const riskLevelOrder = {'R1低风险': 1, 'R2中低风险': 2, 'R3中等风险': 3, 'R4中高风险': 4, 'R5高风险': 5};
                aVal = riskLevelOrder[a.riskLevel] || 0;
                bVal = riskLevelOrder[b.riskLevel] || 0;
                break;
            case 'currentAmount':
...
async function handleWealthSubmit(e) {
    ...
    try {
        ...
        const riskLevel = document.getElementById('wealthRiskLevel').value;
...
async function renderWealthTable() {
    ...
    // 检查是否存在封闭式理财记录
    const hasClosedEndFunds = allCurrentWealth.some(wealth => wealth.type === '封闭式');
    
    // 显示或隐藏到期日期列
    const expiryDateColumn = document.querySelector('#currentFundsTable th:nth-child(6)');
    if (expiryDateColumn) {
        expiryDateColumn.style.display = hasClosedEndFunds ? '' : 'none';
    }
...
async function renderWealthTableSection(tableBodyId, wealthList, tableType, sortedWealth = null) {
    ...
    if (wealthList.length === 0) {
            let colspan = tableType === 'history' ? 8 : 9;
    ...
    const generateTableRows = (wealths) => {
        return wealths.map(wealth => {
            ...
            if (tableType === 'current') {
                ...
                // 检查是否存在封闭式理财记录
                const hasClosedEndFunds = wealthList.some(item => item.type === '封闭式');
                
                // 只有封闭式理财且有多笔交易时显示多行记录，其余类型都只显示一行
                if (wealth.type !== '封闭式' || buyTransactions.length <= 1) {
                    // 只显示一行记录，删除购买金额列
                    return `
                        <tr>
                            ...
                            <td>${wealth.name}</td>
                            <td>${typeDisplay}</td>
                            <td>${wealth.riskLevel || '-'}</td>
                            <td class="amount">${formatAmount(wealth.currentAmount)}</td>
                            ${hasClosedEndFunds ? `<td>${expiryOrCycleDisplay}</td>` : ''}
                            <td>
                        ...
                        if (i === 0) {
                            rowsHtml += `
                                <tr>
                                    ...
                                    <td rowspan="${maxRows}">${wealth.riskLevel || '-'}</td>
    ...
    if (tableType === 'history') {
        ...
        if (finalSortedWealth.length > DEFAULT_DISPLAY_COUNT) {
            ...
            toggleRow.innerHTML = `
                <td colspan="11" style="text-align: center; padding: 10px; background-color: #f5f5f5;">
...
function showYieldCalculationWealthModal(wealth) {
    ...
    return new Promise((resolve) => {
        ...
        const calculateBtn = modal.querySelector('#calculateYieldWealthBtn');
        const closeBtn = modal.querySelector('#closeYieldWealthModalBtn');
        ...
        const handleCalculate = (e) => {
            ...
            const statusEl = modal.querySelector('#yieldCalculationWealthStatus');
    ...
    contentEl.innerHTML = `
        ...
        <div style="margin: 10px 0; padding: 10px; background-color: #f5f5f5; border-radius: 5px;">
            <div style="display: flex; flex-wrap: wrap; gap: 10px; align-items: center;">
                ...
                <div style="flex: 0 0 auto; display: flex; align-items: center; gap: 8px; white-space: nowrap;">
                    <strong>风险等级:</strong> ${wealth.riskLevel || '-'}
                    <button class="btn btn-small btn-edit" onclick="editWealthRiskLevel('${wealth.platform}', '${wealth.name}', '${wealth.type}', '${wealth.riskLevel || ''}')">修改</button>
                </div>
                ${expiryDateInfo}
...
// 存储风险等级编辑的事件处理函数引用
window.editWealthRiskLevelHandlers = {
    confirm: null,
    cancel: null,
    outsideClick: null,
    esc: null
...
// 编辑理财产品风险等级
function editWealthRiskLevel(platform, name, type, currentRiskLevel) {
    // 创建模态框元素
    let modal = document.getElementById('editWealthRiskLevelModal');
    if (!modal) {
        modal = document.createElement('div');
        modal.id = 'editWealthRiskLevelModal';
        modal.className = 'modal';
        modal.innerHTML = `
            <div class="modal-content">
                <h2>编辑风险等级</h2>
                <div style="margin: 15px 0;">
                    <label for="editWealthRiskLevel" style="margin: 0 0 5px 0; display: block;">风险等级:</label>
                    <select id="editWealthRiskLevel" style="width: 100%; padding: 10px; box-sizing: border-box; font-size: 14px; border: 1px solid #ddd; border-radius: 4px;">
                        <option value="">请选择</option>
                        <option value="R1低风险">R1低风险</option>
                        <option value="R2中低风险">R2中低风险</option>
                        <option value="R3中等风险">R3中等风险</option>
                        <option value="R4中高风险">R4中高风险</option>
                        <option value="R5高风险">R5高风险</option>
                    </select>
                </div>
                <div id="editWealthRiskLevelStatus" style="margin: 15px 0; color: red;"></div>
                <div class="form-actions">
                    <button id="editWealthRiskLevelConfirmBtn" class="btn btn-primary">确认修改</button>
                    <button id="editWealthRiskLevelCancelBtn" class="btn btn-secondary">取消</button>
                </div>
            </div>
        `;
        document.body.appendChild(modal);
    ...
    // 设置表单值
    document.getElementById('editWealthRiskLevel').value = currentRiskLevel;
    document.getElementById('editWealthRiskLevelStatus').textContent = '';
    
    // 保存当前编辑的产品信息
    window.currentEditingWealthRiskLevel = {
        platform,
        name,
        type
    };
    ...
    // 获取按钮元素
    const confirmBtn = document.getElementById('editWealthRiskLevelConfirmBtn');
    const cancelBtn = document.getElementById('editWealthRiskLevelCancelBtn');
    
    // 移除之前的事件监听器（使用存储的引用）
    if (window.editWealthRiskLevelHandlers.confirm) {
        confirmBtn.removeEventListener('click', window.editWealthRiskLevelHandlers.confirm);
    }
    if (window.editWealthRiskLevelHandlers.cancel) {
        cancelBtn.removeEventListener('click', window.editWealthRiskLevelHandlers.cancel);
    }
    if (window.editWealthRiskLevelHandlers.outsideClick) {
        modal.removeEventListener('click', window.editWealthRiskLevelHandlers.outsideClick);
    }
    if (window.editWealthRiskLevelHandlers.esc) {
        document.removeEventListener('keydown', window.editWealthRiskLevelHandlers.esc);
    }
    
    // 定义新的事件处理函数
    window.editWealthRiskLevelHandlers.confirm = async () => {
        const newRiskLevel = document.getElementById('editWealthRiskLevel').value;
        const statusEl = document.getElementById('editWealthRiskLevelStatus');
        
        if (!newRiskLevel) {
            statusEl.textContent = '请选择风险等级';
            return;
        }
        
        const { platform, name, type } = window.currentEditingWealthRiskLevel;
        ...
        try {
            ...
            for (let i = 0; i < currentWealth.length; i++) {
                const wealth = currentWealth[i];
                if (wealth.platform === platform && wealth.name === name && wealth.type === type) {
                    wealth.riskLevel = newRiskLevel;
                    // 保存到数据库
                    await fetch(`/api/wealth/${wealth.id}`, {
                        method: 'PUT',
                        headers: {
                            'Content-Type': 'application/json'
                        },
                        body: JSON.stringify({
                            ...
                            transactionType: wealth.transactionType,
                            redeemType: wealth.redeemType,
                            date: wealth.date,
                            expiryDate: wealth.expiryDate,
                            amount: wealth.amount,
                            redeemedAmount: wealth.redeemedAmount,
                            status: wealth.status
                        })
                    });
                }
            ...
            // 刷新交易记录详情弹窗（如果打开）
            const viewModal = document.getElementById('viewWealthTransactionsModal');
            if (viewModal && viewModal.style.display !== 'none') {
                // 重新加载数据后，找到对应的wealth对象
                const mergedWealth = mergeWealth();
                const wealth = mergedWealth.find(w => w.platform === platform && w.name === name && w.type === type);
                if (wealth) {
                    viewWealthTransactions(wealth);
                }
            }
            ...
            // 关闭模态框
            document.getElementById('editWealthRiskLevelModal').style.display = 'none';
            
            // 显示成功提示
            alert('风险等级修改成功！');
        } catch (error) {
            console.error('修改风险等级失败:', error);
            alert('修改失败，请重试！');
        }
    };
    
    window.editWealthRiskLevelHandlers.cancel = () => {
        document.getElementById('editWealthRiskLevelModal').style.display = 'none';
    };
    
    window.editWealthRiskLevelHandlers.outsideClick = (e) => {
        if (e.target === modal) {
            modal.style.display = 'none';
        }
    };
    
    window.editWealthRiskLevelHandlers.esc = (e) => {
        if (e.key === 'Escape') {
            modal.style.display = 'none';
            document.removeEventListener('keydown', window.editWealthRiskLevelHandlers.esc);
            window.editWealthRiskLevelHandlers.esc = null;
        }
    };
    
    // 添加新的事件监听器
    confirmBtn.addEventListener('click', window.editWealthRiskLevelHandlers.confirm);
    cancelBtn.addEventListener('click', window.editWealthRiskLevelHandlers.cancel);
    modal.addEventListener('click', window.editWealthRiskLevelHandlers.outsideClick);
    document.addEventListener('keydown', window.editWealthRiskLevelHandlers.esc);
...
window.editWealthRiskLevel = editWealthRiskLevel;
// 导出理财数据到window对象
window.currentWealth = currentWealth;