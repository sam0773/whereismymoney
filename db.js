...
function initDatabase() {
    ...
    db.run(`
        CREATE TABLE IF NOT EXISTS userSettings (
            username TEXT PRIMARY KEY,
            overviewAmountHidden INTEGER DEFAULT 1, -- 总览金额默认隐藏开关
    ...
    // 如果表已存在但缺少新字段，添加字段（升级现有表）
    db.run('ALTER TABLE userSettings ADD COLUMN overviewAmountHidden INTEGER DEFAULT 1', (err) => {
        if (err && !err.message.includes('duplicate column name')) {
            console.error('添加overviewAmountHidden字段失败:', err.message);
        }
    });
